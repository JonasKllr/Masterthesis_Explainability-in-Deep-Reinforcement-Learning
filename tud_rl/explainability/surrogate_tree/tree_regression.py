...
random_sample_id = np.random.choice(states.shape[0], size=100000, replace=False)
...
if DECISION_TREE:
    ...
    for i in [3, 4, 5, 6]:
        ...
        with open(
            os.path.join(BUFFER_DIR, "trees", f"depth_{i}", "surrogate_tree_model.pkl"),
            "xb",
        ) as file:
    ...
    with open(os.path.join(BUFFER_DIR, "trees", "test_mae.csv"), "xb") as file:
        ...
        PRUNING = False
        if PRUNING:
            path = surrogate_tree.cost_complexity_pruning_path(
            ...
            plt.plot(
                ccp_alphas[:-1], impurities[:-1], marker="o", drawstyle="steps-post"
            )
            ...
            ccp_alphas = np.array(
                [3 * 10 ** (-5), 4 * 10 ** (-5), 4.2 * 10 ** (-5), 4.6 * 10 ** (-5)]
            )

            clfs = []
            for ccp_alpha in ccp_alphas:
                clf = DecisionTreeRegressor(
                    random_state=0, max_depth=40, ccp_alpha=ccp_alpha
                )
